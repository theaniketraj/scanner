...
var getId$2 = (prefix, cnt) => `${prefix}-${cnt}-${Math.random().toString(16).slice(3, 8)}`;
const getId$1 = getId$2;
let jobCounter = 0;
var createJob$1 = ({
  id: _id,
  action,
...
var log$1 = {};
let logging = false;
log$1.logging = logging;
log$1.setLogging = (_logging) => {
  logging = _logging;
};
log$1.log = (...args) => logging ? console.log.apply(void 0, args) : null;
...
var terminateWorker$2 = (worker2) => {
  worker2.terminate();
};
var onMessage$2 = (worker2, handler) => {
  worker2.onmessage = ({ data }) => {
    handler(data);
  };
};
var send$2 = async (worker2, packet) => {
...
let workerCounter = 0;
var createWorker$2 = async (langs = "eng", oem = OEM.LSTM_ONLY, _options = {}, config = {}) => {
...
const recognize = async (image, langs, options) => {
  const worker2 = await createWorker$1(langs, 1, options);
  return worker2.recognize(image).finally(async () => {
    await worker2.terminate();
  });
};
const detect = async (image, options) => {
  const worker2 = await createWorker$1("osd", 0, options);
